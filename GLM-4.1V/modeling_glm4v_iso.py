    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if image_mask_positions is not None:
            ...
            causal_mask[image_mask_positions.unsqueeze(1), image_mask_positions.unsqueeze(0)] = False

            start_idx = 0
            for i, (T, H, W) in enumerate(image_grid_thw):
                num_tokens = H * W // 4
                positions = image_mask_positions[start_idx : start_idx + num_tokens]
                start_idx += num_tokens

                # 图片内部保留原因果（下三角）
                causal_mask[positions.unsqueeze(1), positions.unsqueeze(0)] = torch.tril(
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            image_mask, _, image_mask_positions = self.get_placeholder_mask(input_ids, inputs_embeds, image_features=image_embeds)
