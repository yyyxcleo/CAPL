...
from transformers.models.glm4v.configuration_glm4v import Glm4vConfig, Glm4vTextConfig, Glm4vVisionConfig

from vlmeval.vlm.glm4v.visualize import visualize_selected_tokens
import os
...
class Glm4vTextModel(Glm4vPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        ####
        total_mask = None
        if image_mask_positions is not None and position_ids.size(2) > 1:
            seq_len = position_ids.size(2)
...
class Glm4vModel(Glm4vPreTrainedModel):
    ...
    def get_placeholder_mask(
        ...
        # selection config
        selection_method: str = "l2",   # ["l2", "random"]
        ...
        Returns:
            special_image_mask
            special_video_mask
            image_mask_positions  # 原始global index
            per_image_info        # ⭐结构化结果（推荐使用）
        """

        device = inputs_embeds.device

        # -----------------------------
        # 1. 构造 placeholder mask
        # -----------------------------
        if input_ids is None:
            image_token_embed = self.get_input_embeddings()(
                torch.tensor(self.config.image_token_id, device=device)
            )
            video_token_embed = self.get_input_embeddings()(
                torch.tensor(self.config.video_token_id, device=device)
            )

            special_image_mask = (inputs_embeds == image_token_embed).all(-1)
            special_video_mask = (inputs_embeds == video_token_embed).all(-1)
        else:
            special_image_mask = input_ids == self.config.image_token_id
            special_video_mask = input_ids == self.config.video_token_id  # ✅ 修复

        # -----------------------------
        # 2. token selection + mapping
        # -----------------------------
        ...
        if image_features is not None and image_grid_thw is not None:
            ...
            for img_idx, nt in enumerate(num_tokens_per_image):

                positions_i = all_positions[start:start + nt]
                ...
                if selection_method == "l2":
                    scores = torch.norm(embeds_i, dim=-1)

                elif selection_method == "random":
                    scores = torch.rand(nt, device=device)

                else:
                    raise ValueError(f"Unknown selection method: {selection_method}")

                k = max(1, int(nt * keep_ratio))
                topk_idx = scores.topk(k).indices

                selected_pos = positions_i[topk_idx].sort().values
                image_mask_positions.append(selected_pos)

                # -----------------------------
                # 空间映射
                # -----------------------------
                H = image_grid_thw[img_idx][1].item() // 2
                W = image_grid_thw[img_idx][2].item() // 2
                ...
                per_image_info.append({
                    "image_path": img_path,
                    "num_tokens": nt,
                    "selected_token_count": k,
                    "selected_positions": selected_pos.tolist(),
                    "token_coords": coords,
                    "token_bboxes_norm": bboxes,
                })
            ...
            per_image_info.append({
                "image_path": None,
                "selected_positions": all_positions.tolist(),
                "token_coords": [],
                "token_bboxes_norm": [],
            })
        ...
        if image_features is not None:
            if inputs_embeds[special_image_mask].numel() != image_features.numel():
                raise ValueError("Image features and tokens mismatch")

        if video_features is not None:
            if inputs_embeds[special_video_mask].numel() != video_features.numel():
                raise ValueError("Video features and tokens mismatch")
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            image_mask, _, image_mask_positions, per_image_info = self.get_placeholder_mask(input_ids, inputs_embeds, image_features=image_embeds, image_grid_thw=image_grid_thw, image_paths=image_paths)
